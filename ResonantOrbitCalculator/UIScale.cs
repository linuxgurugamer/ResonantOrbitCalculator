...
namespace ResonantOrbitCalculator
{
    ...
    /// Uses KSP's UI scale settings; when those are at default, gently compensates for resolutions above 1080p.
    /// </summary>
    public static class UIScale
    {
        ...
        public static float Factor
        {
            get
            {
                ...
                if (Screen.height <= 1080)
                    return ksp;
                return Mathf.Max(ksp, Mathf.Min(1.5f, Mathf.Sqrt((float)Screen.height / 1080f)));
            }
        }
        ...
        public static Rect LogicalToScreenRect(Rect logical)
        {
            var factor = Factor;
            if (Mathf.Abs(factor - 1f) < 0.001f)
                return logical;
            return new Rect(
                logical.x * factor,
                logical.y * factor,
                logical.width * factor,
                logical.height * factor);
        }
